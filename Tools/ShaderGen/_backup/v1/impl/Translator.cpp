...
public:
	...
	void Translate( const Options& config, wrSourceFile& srcFile )
	{
		...
		Emit_Declarations( config, srcFile );
		Emit_Definitions( config, srcFile );
	}

	void GetSortedShaders( ParseResults & input )
	{
		Assert(shadersSortedByName.IsEmpty());

		for( UINT iSrcFile = 0;
			iSrcFile < input.parsedFiles.Num();
			iSrcFile++ )
		{
			wrSourceFile& srcFile = input.parsedFiles[ iSrcFile ].fileData;
		...
		if( shadersSortedByName.Num() )
		{
			...
			for( UINT iShader = 0;
				iShader < this->shadersSortedByName.Num();
				iShader++ )
			{
				wrShaderProgram* shader = this->shadersSortedByName[ iShader ];
	...
	=============================================================================
	*/
	void Emit_Declarations( const Options& config, wrSourceFile& srcFile )
	{
		OSFileName		fileName = GetHeaderFileName( config, srcFile );
		...
		Emit_Shared_Sections_Declarations( srcFile.sharedSections, tw );

		Emit_Shader_Declarations( srcFile.shaders, tw );

		Emit_Vertex_Declarations( srcFile.vertexDeclarations, tw );

		//tw	<< MXC("// Function declarations\n");
		//tw.Putf("void %s();\n",GetModuleInitFunctionName(srcFile).ToChars());
		//tw.Putf("void %s();\n",GetModuleShutdownFunctionName(srcFile).ToChars());
	}
	/*
	=============================================================================
	*/
	FixedString GetModuleInitFunctionName( const wrSourceFile& srcFile )
	{
		FixedString	tmp;
	...
	=============================================================================
	*/
	FixedString GetModuleShutdownFunctionName( const wrSourceFile& srcFile )
	{
		FixedString	tmp;
	...
	*/
	...
	{
		OSFileName		fileName = GetSourceFileName( config, srcFile );
		FileWriter		outputStream( fileName.ToChars() );
		MyTextWriter	tw( &outputStream );
		...
		Emit_Shared_Sections_Definitions( srcFile.sharedSections, tw );
		Emit_Shader_Definitions( srcFile.shaders, tw );

		Emit_Create_InputLayouts( srcFile.vertexDeclarations, tw );
		...
		tw.Putf("void %s()\n",GetModuleInitFunctionName(srcFile).ToChars());
		{
			...
			if( srcFile.vertexDeclarations.Num() ) {
	...
	=============================================================================
	*/
	void Emit_Main( const Options& config, const ParseResults& results )
	{
		Emit_Main_Header_File( config, results );
		Emit_Main_Source_File( config, results );
	}
	//===========================================================================
	void Emit_Main_Header_File( const Options& config, const ParseResults& results )
	{
		FileWriter		outputStream( WR_MAIN_HEADER_FILE_NAME );
		MyTextWriter	tw( &outputStream );
	...
	//===========================================================================
	void Emit_Main_Source_File( const Options& config, const ParseResults& results )
	{
		FileWriter		outputStream( WR_MAIN_SOURCE_FILE_NAME );
		MyTextWriter	tw( &outputStream );
		...
		for( UINT iSrcFile = 0;
			...
			iSrcFile++ )
		{
			const wrSourceFile& srcFile = results.parsedFiles[ iSrcFile ].fileData;
		...
		// string table (maps shader indices to their names)
		{
			...
			for( UINT iShader = 0;
				...
				iShader++ )
			{
				const wrShaderProgram* shader = this->shadersSortedByName[ iShader ];
				tw.Putf( "%s::Name,\n", Get_ShaderClass_Name(*shader) );
			}
		}
	...
	*/
	void Emit_Initialize_RenderTargets( TList< wrRenderTarget >& renderTargets, MyTextWriter & tw )
	{
		...
		tw	<< MXC("static void SetupRenderTargets()\n");
		wrScope	sc(tw);
	...
	void Emit_Initialize_RenderTarget( const wrRenderTarget& renderTarget, MyTextWriter & tw )
	{
		...
		tw	.Putf( "texDesc.Width				= %s;\n", Get_RT_Size( renderTarget.sizeX, MXC("GetBackbufferWidth()") ).ToChars() );
		tw	.Putf( "texDesc.Height				= %s;\n", Get_RT_Size( renderTarget.sizeY, MXC("GetBackbufferHeight()") ).ToChars() );
		...
		tw	.Putf("graphics.resources->Create_RenderTarget( texDesc, rtvDesc, srvDesc, %s%s );\n",
			WR_RenderTarget_PREFIX, renderTarget.name.ToChars() );
	...
	void Emit_Initialize_SamplerState( wrSamplerState& samplerState, MyTextWriter & tw )
	{
		...
		tw	.Putf("graphics.resources->Create_SamplerState( samplerDesc, %s%s );\n",
			WR_SamplerState_PREFIX, samplerState.name.ToChars() );
	}
	//===========================================================================
	...
	void Emit_Initialize_DepthStencilState( wrDepthStencilState& depthStencilState, MyTextWriter & tw )
	{
		...
		tw	.Putf("graphics.resources->Create_DepthStencilState( depthStencilDesc, %s%s );\n",
			WR_DepthStencilState_PREFIX, depthStencilState.name.ToChars() );
	}
	//===========================================================================
	...
	void Emit_Initialize_RasterizerState( wrRasterizerState& rasterizerState, MyTextWriter & tw )
	{
		...
		tw	.Putf("graphics.resources->Create_RasterizerState( rasterizerDesc, %s%s );\n",
			WR_RasterizerState_PREFIX, rasterizerState.name.ToChars() );
	}
	//===========================================================================
	...
	void Emit_Initialize_BlendState( wrBlendState& blendState, MyTextWriter & tw )
	{
		...
		tw	.Putf("graphics.resources->Create_BlendState( blendDesc, %s%s );\n",
			WR_BlendState_PREFIX, blendState.name.ToChars() );
	}
	//===========================================================================
	...
	void Emit_Release_Resources( const wrSourceFile& srcFile, MyTextWriter & tw )
	{
		...
		if( srcFile.vertexDeclarations.Num() )
		{
			...
			for( UINT iVtx = 0;
				iVtx < srcFile.vertexDeclarations.Num();
				iVtx++ )
			{
				const wrVertexDeclaration& vtx = srcFile.vertexDeclarations[ iVtx ];

				tw.Putf("graphics.resources->Destroy_InputLayout( %s::IL );\n", vtx.name.ToChars() );
			}
		}
	}
	//===========================================================================
	...
	void Emit_Shader_Declaration( const wrShaderProgram & shader, MyTextWriter & tw )
	{
		...
		Emit_Struct( Get_ShaderClass_Name(shader), tw );
		{
			...
			tw.Putf("enum { UID = %u }; // unique index\n",shader.uniqueIndex);
			...
			tw << "static void Load( const ShaderInfo& shaderInfo, ShaderManager* compiler );\n";
...
L_End:
		...
		//Initialize
		{
			...
			tw.NewLine();
			tw.Putf("%s::Load( d, graphics.shaders );\n",shaderClassName);
		}//Initialize
...
*/
		...
		{
			tw.Putf("void %s::GetLoadInfo( ShaderInfo & outInfo )\n"
				,shaderClassName);
			wrScope	sc(tw);
		...
		//Load()
		{
			tw.Putf("void %s::Load( const ShaderInfo& shaderInfo, ShaderManager* compiler )\n"
				,Get_ShaderClass_Name(shader));
			wrScope	sc(tw);
...
//===========================================================================
	void Emit_Shared_Section_Declaration( const wrSharedSection& sharedSection, MyTextWriter & tw )
	{
		...
		Emit_Struct( Get_Shared_Section_Name(sharedSection.name.ToChars()), tw );
		{
			...
			Emit_Shared_Section_Public_Methods_Declarations(sharedSection,tw);
		}
	...
	void Emit_Shared_Section_Public_Methods_Declarations( const wrSharedSection& sharedSection, MyTextWriter & tw )
	{
		tw << "static inline void Set()\n";
		wrScope	functionScope(tw);
	...
	void Emit_Shared_Section_Definition( const wrSharedSection& sharedSection, MyTextWriter & tw )
	{
		...
		Emit_Shader_Variables_As_Struct_Members_Definitions(sharedSection.vars,tw,Get_Shared_Section_Name(sharedSection.name.ToChars() ));
	}
	//===========================================================================
	...
	void Emit_Create_InputLayout(const wrVertexDeclaration& vtx, MyTextWriter & tw)
	{
		...
		tw.Putf("graphics.resources->Create_InputLayout( vtxFormat.elements.ToPtr(), vtxFormat.elements.Num(), %s::IL );\n", vtx.name.ToChars());
	}
	//===========================================================================

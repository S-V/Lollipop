...
void Parser::shader_variable_decl() {
		if (la->kind == 8) {
...
void Parser::constant_buffer_decl() {
		wrShaderConstantBuffer& newCB = ParsedFile().NewConstantBuffer( this ); SETPOS(newCB); 
		Expect(8);
		Expect(1);
		newCB.name = TokenBuf(); 
		if (la->kind == 30) {
...
void Parser::sampler_state_decl() {
		wrShaderSamplerState& newSS = ParsedFile().NewSamplerState( this ); SETPOS(newSS); 
		Expect(46);
		Expect(1);
		newSS.name = TokenBuf(); 
		if (la->kind == 30) {
...
void Parser::shader_resource_decl() {
		wrShaderResource& newRes = ParsedFile().NewTexture( this ); SETPOS(newRes); 
		Expect(88);
		Expect(1);
...
void Parser::constant_buffer_register() {
		Expect(30);
		Expect(18);
		Expect(32);
		Expect(3);
		ParsedFile().LastConstantBuffer().iRegister = TokenToU32(0,MAX_CONSTANT_BUFFER_SLOTS-1); 
		Expect(33);
}

void Parser::constant_buffer_element() {
		wrCBVar& newCBElem = ParsedFile().LastConstantBuffer().elements.Add(); SETPOS(newCBElem); 
		type_name();
...
bool Parser::StartOf(int s) {
	...
	static bool set[2][91] = {
		...
		{x,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,T,T, T,T,x}
	};
...
void Errors::SynErr(int line, int col, int n) {
	...
	switch (n) {
			...
			case 90: s = coco_string_create(L"invalid declaration"); break;
			case 91: s = coco_string_create(L"invalid render_target_declaration"); break;
			case 92: s = coco_string_create(L"invalid render_target_declaration"); break;
			case 93: s = coco_string_create(L"invalid boolean_constant"); break;
			case 94: s = coco_string_create(L"invalid bit_mask"); break;
			case 95: s = coco_string_create(L"invalid shader_variable_decl"); break;

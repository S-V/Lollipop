...
public:
	explicit MyTextWriter( AStreamWriter& stream )
	...
	virtual void Put( const char* s, UINT length ) override
	{
		if( scopeDepth > 0 ) {
...
//===========================================================================
	...
	*/
	void Emit_Main( const Options& config, const wrShaderLibrary& shaderLib )
	{
		...
		Emit_Vertex_Formats_Declarations_Inline_File( config, shaderLib );
	...
	void Emit_Vertex_Formats_Declarations_Header_File( const Options& config, const wrShaderLibrary& shaderLib )
	{
		...
		if( config.bEmitVertexFormatsDecl ) {
			fullPath = config.outputFolderCPP;
		} else {
	...
	//===========================================================================
	void Emit_Vertex_Formats_Declarations_Inline_File( const Options& config, const wrShaderLibrary& shaderLib )
	{
		OSPathName		fullPath;
		...
		fullPath.Append( MXC("VertexFormats.inl") );
			...
			iVtx++ )
		{
			...
			tw.Putf("RX_PROCESS_VERTEX_TYPE_WITH_MACRO( %s )\n",
				vtx.name.ToChars());
		}
	...
	void Emit_Shader_Programs_Declarations_Header_File( const Options& config, const wrShaderLibrary& shaderLib )
	{
		...
		if( config.bEmitComments || config.bDebugMode ) {
			tw.Putf("// %u shader programs (%u combinations)\n\n",
	...
	void Emit_Main_Header_File( const Options& config, const wrShaderLibrary& shaderLib )
	{
		...
		if( config.bEmitComments )
		{
			...
			tw.Putf(
				...
				"// %u rasterizer states\n"
				"// %u blend states\n"
				"// %u shader programs (%u combinations)\n"
				"// %u input layouts\n"
				"\n"
				,
	...
	void Emit_Main_Initialize(const Options& config, const wrShaderLibrary& shaderLib, MyTextWriter & tw)
	{
		...
		tw	<< MXC("rxGPU_MARKER( Create_Shaders );\n\n");
	...
	*/
	...
	=============================================================================
	*/
	void Emit_Add_Render_Targets_To_Debug_Views( const TList< wrRenderTarget >& renderTargets, MyTextWriter & tw )
	{
		const UINT numRenderTargets = renderTargets.Num();
		...
		tw << MXC("#if MX_EDITOR\n");
		{
				...
				iRenderTarget++ )
			{
				...
				StackString	renderTargetName;
				renderTargetName.SetString(Get_RenderTarget_Name(renderTarget));

				tw.Putf( "gRenderer.editor->GetTextureInspector()->AddDebugItem( %s.pTexture, \"%s\" );\n",
					renderTargetName.ToChars(), renderTargetName.ToChars() );
			}
		}
		tw << MXC("#endif // MX_EDITOR\n");
	}
	/*
	=============================================================================
	*/
	void Emit_Remove_Render_Targets_From_Debug_Views( const TList< wrRenderTarget >& renderTargets, MyTextWriter & tw )
	{
		const UINT numRenderTargets = renderTargets.Num();
		...
		tw << MXC("#if MX_EDITOR\n");
		{
				...
				iRenderTarget++ )
			{
				...
				tw.Putf( "gRenderer.editor->GetTextureInspector()->RemoveDebugItem( %s.pTexture );\n",
					renderTargetName.ToChars() );
			}
		}
		tw << MXC("#endif // MX_EDITOR\n");
	}
	...
	*/
	void Emit_Create_RenderTargets( const TList< wrRenderTarget >& renderTargets, MyTextWriter & tw )
	{
		...
		tw	<< MXC("static void SetupRenderTargets( const UINT sizeX, const UINT sizeY )\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Create_Render_Targets );\n\n");
			...
			Emit_Add_Render_Targets_To_Debug_Views( renderTargets, tw );
		}
		...
		tw	<< MXC("void ReleaseRenderTargets()\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Render_Targets );\n\n");

			Emit_Remove_Render_Targets_From_Debug_Views( renderTargets, tw );
		...
		tw	<< MXC("void ReallocRenderTargets( UINT sizeX, UINT sizeY )\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Realloc_Render_Targets );\n\n");
	...
	void Emit_Create_SamplerStates( const TList< wrSamplerState >& samplerStates, MyTextWriter & tw )
	{
		...
		tw	<< MXC("static void SetupSamplerStates()\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Create_Sampler_States );\n\n");
	...
	void Emit_Create_DepthStencilStates( const TList< wrDepthStencilState >& depthStencilStates, MyTextWriter & tw )
	{
		...
		tw	<< MXC("static void SetupDepthStencilStates()\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Create_DepthStencil_States );\n\n");
	...
	wrName Get_Stencil_Func_D3D11( const wrName& stencilFunc )
	{
		wrBuffer	tmp = ToUpper( stencilFunc );
		...
		Unreachable;
		return wrName("D3D11_COMPARISON_NEVER");
	}

	wrName Get_Stencil_Op_D3D11( const wrName& stencilFunc )
	{
		wrBuffer	tmp = ToUpper( stencilFunc );
	...
	void Emit_Create_DepthStencilState( const wrDepthStencilState& depthStencilState, MyTextWriter & tw )
	{
		...
		tw	.Putf("depthStencilDesc.FrontFace.StencilPassOp			= %s;\n", Get_Stencil_Op_D3D11( depthStencilState.FrontFaceStencilPass ).ToChars());
		tw	.Putf("depthStencilDesc.FrontFace.StencilFunc			= %s;\n", Get_Stencil_Func_D3D11( depthStencilState.FrontFaceStencilFunc ).ToChars());

		tw	<< MXC("depthStencilDesc.BackFace.StencilFailOp			= D3D11_STENCIL_OP_KEEP;\n")
	...
	void Emit_Create_RasterizerStates( const TList< wrRasterizerState >& rasterizerStates, MyTextWriter & tw )
	{
		...
		tw	<< MXC("static void SetupRasterizerStates()\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Create_Rasterizer_States );\n\n");
	...
	void Emit_Create_BlendStates( const TList< wrBlendState >& blendStates, MyTextWriter & tw )
	{
		...
		tw	<< MXC("static void SetupBlendStates()\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Create_Blend_States );\n\n");
	...
	void Emit_Release_Resources( const wrShaderLibrary& shaderLib, MyTextWriter & tw )
	{
		if( shaderLib.shaders.Num() )
		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Shaders );\n\n");
		...
		if( shaderLib.samplerStates.Num() )
		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Sampler_States );\n\n");
		...
		if( shaderLib.depthStencilStates.Num() )
		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Depth_Stencil_States );\n\n");
		...
		if( shaderLib.rasterizerStates.Num() )
		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Rasterizer_States );\n\n");
		...
		if( shaderLib.blendStates.Num() )
		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Blend_States );\n\n");
		...
		if( shaderLib.renderTargets.Num() )
		{
			Emit_Remove_Render_Targets_From_Debug_Views( shaderLib.renderTargets, tw );
		...
		if( shaderLib.vertexDeclarations.Num() )
		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Input_Layouts );\n\n");
...

		{
			...
			tw	<< MXC("rxGPU_MARKER( Release_Shared_Sections );\n\n");
	...
	void Emit_Shader_Definition( const wrShaderProgram& shader, MyTextWriter & tw )
	{
		...
		mxOPTIMIZE("shader instances should be grouped together (CPU cache)");
...
//===========================================================================
	...
	void Emit_Shared_Sections_Definitions( const TList< wrSharedSection > & sharedSections, MyTextWriter & tw )
	{
		...
		tw << "static void SetupSharedSections()\n";
		{
			...
			tw	<< MXC("rxGPU_MARKER( Create_Shared_Sections );\n\n");
			...
			);
		{
				...
				iVertexStream++ )
			{
					...
					iStreamElement++ )
				{
					...
					tw.Putf("TCopyVertices( c_cast(%s*)dest.streams[ %s ].ToVoidPtr(), src.%s,\n",
	...
	void Emit_Create_InputLayouts( const TList< wrVertexDeclaration >& vertexDecls, MyTextWriter & tw )
	{
		...
		tw.Putf("static void CreateInputLayouts()\n");
		{
			...
			tw	<< MXC("rxGPU_MARKER( Create_Input_Layouts );\n\n");
	...
	void Emit_Bind_Constant_Buffers( const TList< wrShaderConstantBuffer >& constantBuffers, MyTextWriter & tw )
	{
			...
			iCB++ )
		{
			...
			if( cb.usedByVS() )
			{
				...
				tw << "theContext->VSSetConstantBuffers(\n";
				tw.Putf("\t%u,\n",cb.iRegister);
				tw.Putf("\t%u,\n",1);
				tw.Putf("\t(ID3D11Buffer**)&%s\n",Get_ShaderConstantBuffer_Name(cb));
				tw << ");\n";
			...
			if( cb.usedByPS() )
			{
				...
				tw << "theContext->PSSetConstantBuffers(\n";
				tw.Putf("\t%u,\n",cb.iRegister);
				tw.Putf("\t%u,\n",1);
				tw.Putf("\t(ID3D11Buffer**)&%s\n",Get_ShaderConstantBuffer_Name(cb));
				tw << ");\n";
	...
	void Emit_Cpp_code_for_assertions( const char* structName, const wrShaderVariables& vars, MyTextWriter& tw )
	{
		...
		if( vars.samplers.Num() > 1 )
		{
				...
				iSamplerState++ )
			{
				...
				tw.Putf("mxENSURE( samplerStatesArray + %u == &%s::%s );\n"
					,iSamplerState
					,structName
					,ss.name.ToChars()
				);
		...
		if( vars.resources.Num() > 1 )
		{
				...
				iShaderResource++ )
			{
				...
				tw.Putf("mxENSURE( shaderResourcesArray + %u == &%s::%s );\n"
					,iShaderResource
					,structName
					,res.name.ToChars()
				);

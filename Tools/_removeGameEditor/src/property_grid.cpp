...
(void)pPropertyArray;
	...
	QWidget* CreatePropertyEditor( AEditableProperty* theProperty, QWidget* parent )
	{
		...
		//{
		//	EdProperty_AssetReference* pProperty = static_cast< EdProperty_AssetReference* >( theProperty );
		//	ResourceRefPropertyEditor* pEditor = new ResourceRefPropertyEditor( pProperty, parent );
		//	return pEditor;
		//}
		else if (theProperty->Is<EdProperty_String>())
		{
...
/*
-----------------------------------------------------------------------------
	ResourceRefPropertyEditor
-----------------------------------------------------------------------------
*/
ResourceRefPropertyEditor::ResourceRefPropertyEditor( EdProperty_AssetReference* theProperty, QWidget* parent )
	: Super( parent )
	, APropertyEditor( theProperty )
{
	m_property = theProperty;
...
// reads data from the model and writes it to the editor widget.
void ResourceRefPropertyEditor::SetEditorData()
{
	const SResourceObject* resource = m_property->Get();
	if( resource != nil )
	{
		this->setText( resource->edGetEditorInterface()->edGetName() );
	}
}
// reads the contents of the editor, and writes it to the model.
void ResourceRefPropertyEditor::SetModelData()
{
	...
	{
		ObjectGUID resGuid = gCore.resources->edGetResourceGuidByName( this->text().toAscii().data() );
		ObjectID resId = gCore.resources->GetResourceHandleByGuid( m_property->GetBaseResourceType(), resGuid );
		SResourceObject* pResource = gCore.resources->GetResource( resId );

		m_property->Set( pResource );
	}
}

void ResourceRefPropertyEditor::dragEnterEvent(QDragEnterEvent* theEvent)
{
...
void ResourceRefPropertyEditor::dropEvent(QDropEvent* theEvent)
{
	const EdMimeData* myData =
		qobject_cast< const EdMimeData* >( theEvent->mimeData() );
...
void ResourceRefPropertyEditor::mouseDoubleClickEvent( QMouseEvent* theEvent )
{
...
#if 0
	ProperyEditorDockWidget* newPropsEditor = new ProperyEditorDockWidget();
	newPropsEditor->SetObject( pResource );

	//GetMainFrame()->addDockWidget(Qt::RightDockWidgetArea,newPropsEditor);
	GetMainFrame()->addDockWidget(Qt::LeftDockWidgetArea,newPropsEditor);
	newPropsEditor->show();
#else
	Util_NewPropertyEditorDockWidget( pResource->edGetEditorInterface(), this );
#endif
}
#endif
	...
	, APropertyEditor( theProperty )
{
	...
	const SEnumReflection* pEnumReflection = m_property->GetEnumReflection();
	for (UINT iEnumItem = 0; iEnumItem < pEnumReflection->count; iEnumItem++ )
	{
		const SEnumItem& rEnumItem = pEnumReflection->items[ iEnumItem ];
		this->addItem(rEnumItem.name.ToChars(), QVariant(rEnumItem.value));
	}
...
{
	const SEnumItem& currItem = m_property->GetCurrEnumItem();
	...
	: Super( parent )
{
	...
	MX_CONNECT_THIS( EdSystem::Get().Event_ObjectBeingDestroyed, ProperyEditorWidget, _OnDataBeingDestroyed );
...
void ProperyEditorWidget::dropEvent( QDropEvent* theEvent )
{
	...
	const EdMimeData* pMimeData = qobject_cast< const EdMimeData* >( theEvent->mimeData() );

	if( pMimeData != nil )
	{
		if( !pMimeData->objects.IsEmpty() )
		{
			UNDONE;
			//theObject->edBulkAdd( pMimeData->objects.ToPtr(), pMimeData->objects.Num() );

			this->expand(this->indexAt( theEvent->pos() ));

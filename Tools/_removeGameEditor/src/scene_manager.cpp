...
void Ed_OnNewWorldCreated( World* newWorld )
{
	AssertPtr(newWorld);
	...
	//{
	//	rxParallelLight* pNewLight = newWorld->m_renderWorld.CreateDirectionalLight();
	//	pNewLight->SetDiffuseColor( (FColor::WHITE * 0.3f).AsRGB() );
	//	(void)pNewLight;
	//}

	//{rxLocalLight* pNewLight = newWorld->m_renderWorld.CreateLocalLight();
	//(void)pNewLight;}
	...
	UNDONE;
	//EdSystem::Get().Notify_WorldLoaded( newWorld );
	//EdSystem::Get().Notify_SelectionChanged( newWorld );
}
...
void EdSceneTreeView::OnItemSelected( AEditable* pObject )
{
	...
	if( EdModeTools::HasInstance() )
	{
		EdModeTools& editorTools = EdModeTools::Get();
		editorTools.OnSelectionChangedInSceneTreeView( pObject );
	}
}
...
void EdSceneManager::ConnectSigSlots()
{
	...
	MX_CONNECT_THIS( EdSystem::Get().Event_UpdateState, EdSceneManager, UpdateAll );
}
...
void EdSceneManager::slot_CreateNewWorld()
{
	...
	Global_Level_Editor::Get().CreateNewWorld();
}

...
void AssetsTreeView::contextMenuEvent( QContextMenuEvent* theEvent )
{
	...
	else
	{
		...
		qtCONNECT(
			menu.addAction("Create Material"), SIGNAL(triggered()),
			this, SLOT(slot_Create_Material())
		);
...
mxREMOVE_OLD_CODE
void AssetsTreeView::slot_Create_Material()
{
	UNDONE;
	//this->file
}
	...
	: QDockWidget( parent )
{
	...
	mxCONNECT_THIS( EdSystem::Get().Event_AssetDbChanged, AssetsViewDockWidget, OnAssetDatabaseChanged );
}

AssetsViewDockWidget::~AssetsViewDockWidget()
{
	mxDISCONNECT_THIS( EdSystem::Get().Event_AssetDbChanged );
}

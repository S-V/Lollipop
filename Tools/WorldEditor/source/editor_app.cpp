...
EdApp::EdApp()
{
	...
	// Initialize modules.
	{
		...
		// read editor_app settings from file
...
void EdApp::ConnectSigSlots()
{
	...
	mxCONNECT_THIS( EdSystem::Get().Event_ProjectLoaded, EdApp, slot_OnProjectLoaded );
	mxCONNECT_THIS( EdSystem::Get().Event_ProjectUnloaded, EdApp, slot_OnProjectUnloaded );
}
...
void EdApp::UpdateAll()
{
...

	{
		...
		// if no projects are loaded, then draw an empty viewport

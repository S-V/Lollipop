...
bool EdProject::Save()
{
	...
	{
		MemoryBlob			memBlob(EMemHeap::HeapTemp);
		MemoryBlobWriter	stream(memBlob);
		...
		if(1)
		{
			const UINT numWorlds = Universe_Editor::Get().Num();
		...
		CHK_VRET_FALSE_IF_NOT( serializer.WriteAllToStream( stream ) );

		ANSICHAR	levelFilePath[FS_MAX_PATH];
		F_Compose_File_Path( m_fullFilePath, PROJ_LEVEL_FILE_EXTENSION, levelFilePath );

		CHK_VRET_FALSE_IF_NOT( Util_SaveBlobToFile( levelFilePath, memBlob ) );
	}
	...
	// Serialize project data
	{
		...
		CHK_VRET_FALSE_IF_NOT(serializer.WriteAllToStream( fileStream ));
	}
...
bool EdProject::Load()
{
	...
	// deserialize project data
	{
		...
		TextReader	serializer( fileStream );
		{
			{
				serializer.Serialize_String( "Path_To_Asset_Database", m_assetDbFilePath );
				if( !m_assetDbFilePath.IsEmpty() )
...
#if MX_DEVELOPER
	mxCONNECT_THIS( EdSystem::Get().Event_AppInitialized, EdProjectManager, OpenMostRecentlyUsedFile );
#endif//MX_DEVELOPER
}
//-----------------------------------------------------------------------------
...
void EdProjectManager::CloseCurrentProject()
{
	...
	EditorApp::GetMainFrame()->setWindowTitle( QString(ED_APP_NAME) );
}
//-----------------------------------------------------------------------------
...
bool EdProjectManager::OpenProjectFile( const String& filePath )
{
	...
	EditorApp::GetMainFrame()->setWindowTitle( QString(ED_APP_NAME) + " - " + m_currentProject->GetName() );
...
void ProjectInfoPage::selectProjectDir()
{
	...
	if (dialog.exec())
	{
		...
		F_NormalizePath( EdSystem::Get().settings.pathToProjects );
	}
}
//-----------------------------------------------------------------------------

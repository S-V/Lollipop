...
EdTreeViewModel::EdTreeViewModel( AEditable* root )
{
	...
	mxCONNECT_THIS( EdSystem::Get().Event_ObjectModified, EdTreeViewModel, OnDataModified );
	mxCONNECT_THIS( EdSystem::Get().Event_ObjectBeingDestroyed, EdTreeViewModel, OnDataBeingDestroyed );

	mxCONNECT_THIS( EdSystem::Get().Event_BeginInsertRows, EdTreeViewModel, OnBeginInsertRows );
	mxCONNECT_THIS( EdSystem::Get().Event_EndInsertRows, EdTreeViewModel, OnEndInsertRows );

	mxCONNECT_THIS( EdSystem::Get().Event_BeginRemoveRows, EdTreeViewModel, OnBeginRemoveRows );
	mxCONNECT_THIS( EdSystem::Get().Event_EndRemoveRows, EdTreeViewModel, OnEndRemoveRows );

	mxCONNECT_THIS( EdSystem::Get().Event_BeginResetModel, EdTreeViewModel, OnBeginResetModel );
	mxCONNECT_THIS( EdSystem::Get().Event_EndResetModel, EdTreeViewModel, OnEndResetModel );
}
...
EdTreeViewModel::~EdTreeViewModel()
{
	...
	mxDISCONNECT_THIS( EdSystem::Get().Event_ObjectModified );
	mxDISCONNECT_THIS( EdSystem::Get().Event_ObjectBeingDestroyed );

	mxDISCONNECT_THIS( EdSystem::Get().Event_BeginInsertRows );
	mxDISCONNECT_THIS( EdSystem::Get().Event_EndInsertRows );

	mxDISCONNECT_THIS( EdSystem::Get().Event_BeginRemoveRows );
	mxDISCONNECT_THIS( EdSystem::Get().Event_EndRemoveRows );

	mxDISCONNECT_THIS( EdSystem::Get().Event_BeginResetModel );
	mxDISCONNECT_THIS( EdSystem::Get().Event_EndResetModel );
}
...
QModelIndex EdTreeViewModel::parent(const QModelIndex &child) const
{
	...
	mxDBG_CHECK_VTBL(childItem);
	//qDebug( "TreeModel: %s\n", this->metaObject()->className() );
	//DBGOUT("\n**** childItem addr = 0x%x\n",childItem);

	AEditable* parentItem = childItem->edGetParent();

	mxDBG_CHECK_VTBL(parentItem);
...
void EdTreeView::contextMenuEvent( QContextMenuEvent * theEvent )
{
	...
	if( pObject != nil )
	{
		...
		//args.x = theEvent->globalPos().x();
		//args.y = theEvent->globalPos().y();

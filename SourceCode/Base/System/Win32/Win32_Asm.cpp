...
// 'buffer' should be 16-byte aligned in offset
// 'size' should be padded to 64 bytes because of loop unrolling
void MemsetSSE2( UINT32* buffer, UINT32 sizeBytes, INT32 value )
{
	Assert((sizeBytes % 64) == 0);
	Assert(((UINT)buffer % 16) == 0);

	// use SSE2, 128bit integer writes
	const UINT32 size = sizeBytes / sizeof(UINT32);
	const __m128i _v = _mm_set1_epi32(value);

	for( UINT32 i = 0; i < size; i += 16, buffer += 16 )
	{
		_mm_stream_si128((__m128i*)(buffer), _v);
		_mm_stream_si128((__m128i*)(buffer + 4), _v);
		_mm_stream_si128((__m128i*)(buffer + 8), _v);
		_mm_stream_si128((__m128i*)(buffer + 12), _v);
	}
}
...
mxSWIPED("PCSX2 - PS2 Emulator for PCs, see: http://pcsx2.googlecode.com"
"not needed anymore, use memcpy intrinsic - expands to asm even in debug mode"
);

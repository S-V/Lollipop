...
void idLexer::CreatePunctuationTable( const punctuation_t *punctuations ) {
	...
	if ( punctuations == default_punctuations ) {
		...
		if ( !idLexer::punctuationtable || idLexer::punctuationtable == default_punctuationtable ) {
			idLexer::punctuationtable = (int *) mxAlloc(256 * sizeof(int));
		}
		if ( idLexer::nextpunctuation && idLexer::nextpunctuation != default_nextpunctuation ) {
			mxFree( idLexer::nextpunctuation );
		}
		for (i = 0; punctuations[i].p; i++) {
		}
		idLexer::nextpunctuation = (int *) mxAlloc(i * sizeof(int));
	}
...
void idLexer::Error( const char *str, ... ) {
	...
	if ( idLexer::flags & LEXFL_NOFATALERRORS ) {
		mxWarnf( "file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text );
	} else {
		mxErrf( "file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text );
	}
}
...
================
*/
FORCEINLINE int idLexer::CheckString( const char *str ) const {
	int i;
...
void idLexer::UnreadToken( const idToken *token ) {
	if ( idLexer::tokenavailable ) {
		mxFatal( "Parser::unreadToken, unread token twice\n" );
	}
	idLexer::token = *token;
...
================
*/
const char*	idLexer::ReadRestOfLine(String& out) {
	while(1) {
...
=================
*/
const char *idLexer::ParseBracedSectionExact( String &out, int tabs ) {
	int		depth;
	bool	doTabs;
	bool	skipWhite;
...
=================
*/
const char *idLexer::ParseBracedSection( String &out ) {
	idToken token;
	int i, depth;
...
=================
*/
const char *idLexer::ParseRestOfLine( String &out ) {
	idToken token;
...
int idLexer::LoadFile( const char *filename ) {
	...
	return false;
	/*
	mxFile_ReadOnly file;
	String pathname;
	int length;
	char *buf;

	if ( idLexer::loaded ) {
		Error("Parser::LoadFile: another script already loaded");
		return false;
	}
	...
	length = file.GetSize();
	buf = (char *) mxAlloc( length + 1 );
	buf[length] = '\0';
	file.Read( buf, length );
	idLexer::fileTime = file.Timestamp();
	idLexer::filename = file.GetName();
...
int idLexer::LoadMemory( const char *ptr, int length, const char *name, int startLine ) {
	if ( idLexer::loaded ) {
		Error("Parser::LoadMemory: another script already loaded");
		return false;
	}
...
#ifdef PUNCTABLE
	if ( idLexer::punctuationtable && idLexer::punctuationtable != default_punctuationtable ) {
		mxFree( (void *) idLexer::punctuationtable );
		idLexer::punctuationtable = NULL;
	}
	if ( idLexer::nextpunctuation && idLexer::nextpunctuation != default_nextpunctuation ) {
		mxFree( (void *) idLexer::nextpunctuation );
		idLexer::nextpunctuation = NULL;
	}
#endif //PUNCTABLE
	if ( idLexer::allocated ) {
		mxFree( (void *) idLexer::buffer );
		idLexer::buffer = NULL;
		idLexer::allocated = false;
	}
	idLexer::tokenavailable = 0;
...
================
*/
idLexer::idLexer( const char *filename, int flags ) {
	idLexer::loaded = false;
	idLexer::flags = flags;
	idLexer::SetPunctuations( NULL );
	idLexer::allocated = false;

...
public:
					...
					idLexer();
					idLexer( int flags );
					idLexer( const char *filename, int flags = 0 );
	...
	const char *	ParseBracedSection( String &out );
	...
	void			Error( const char *str, ... ) mxCHECK_PRINTF_ARGS;
					// print a warning message
	void			Warning( const char *str, ... ) mxCHECK_PRINTF_ARGS;
...
private:
	...
	shared_ char	baseFolder[ 256 ];		// base folder to load files from
...
FORCEINLINE const char *idLexer::GetFileName( void ) const {
	return idLexer::filename;
}

FORCEINLINE const int idLexer::GetFileOffset( void ) const {
	return idLexer::script_p - idLexer::buffer;
}

FORCEINLINE const unsigned int idLexer::GetFileTime( void ) const {
	return idLexer::fileTime;
}

FORCEINLINE const int idLexer::GetLineNum( void ) const {
	return idLexer::line;
}

FORCEINLINE void idLexer::SetFlags( int flags ) {
	idLexer::flags = flags;
}

FORCEINLINE int idLexer::GetFlags( void ) const {

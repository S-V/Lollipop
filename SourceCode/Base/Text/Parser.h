...
public:
					...
					idParser();
					idParser( int flags );
					idParser( const char *filename, int flags = 0 );
	...
	const char *	ParseBracedSection( String &out, int tabs = -1 );
					// parse a braced section into a string, maintaining indents and newlines
	const char *	ParseBracedSectionExact( String &out, int tabs = -1 );
					// parse the rest of the line
	const char *	ParseRestOfLine( String &out );
	...
	void			Error( const char *str, ... ) const mxCHECK_PRINTF_ARGS;
					// print a warning message
	void			Warning( const char *str, ... ) const mxCHECK_PRINTF_ARGS;
...
private:
	...
	shared_ define_t *globaldefines;				// list with global defines added to every source loaded
...
FORCEINLINE const char *idParser::GetFileName( void ) const {
...
FORCEINLINE const int idParser::GetFileOffset( void ) const {
	if ( idParser::scriptstack ) {
...
FORCEINLINE const unsigned int idParser::GetFileTime( void ) const {
	if ( idParser::scriptstack ) {
...
FORCEINLINE const int idParser::GetLineNum( void ) const {
	if ( idParser::scriptstack ) {

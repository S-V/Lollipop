...
template<class T> inline void AssertValidReadPtr( T* ptr, int count = 1 )		     { AssertPtr( (void*)ptr ); }
template<class T> inline void AssertValidWritePtr( T* ptr, int count = 1 )		     { AssertPtr( (void*)ptr ); }
template<class T> inline void AssertValidReadWritePtr( T* ptr, int count = 1 )	     { AssertPtr( (void*)ptr ); }
...
char *V_pretifymem( float value, int digitsafterdecimal /*= 2*/, bool usebinaryonek /*= false*/ )
{
	...
	if ( mxFabs( value - (int)value ) < 0.00001 )
	{
...
void V_AppendSlash( char *pStr, int strSize )
{
	...
	{
		if ( len+1 >= strSize )
			mxErrf( "V_AppendSlash: ran out of space on %s.", pStr );
...
void V_MakeAbsolutePath( char *pOut, int outLen, const char *pPath, const char *pStartingDir )
{
	...
	else
	{
		...
		else
		{
			if ( !_getcwd( pOut, outLen ) )
				mxErrf( "V_MakeAbsolutePath: _getcwd failed." );
	...
	if ( !V_RemoveDotSlashes( pOut ) )
		mxErrf( "V_MakeAbsolutePath: tried to \"..\" past the root." );

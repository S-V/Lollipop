...
void idParser::PrintDefine( define_t *define ) {
	mxPutf("define->name = %s\n", define->name);
	mxPutf("define->flags = %d\n", define->flags);
	mxPutf("define->builtin = %d\n", define->builtin);
	mxPutf("define->numparms = %d\n", define->numparms);
}
...
================
*/
FORCEINLINE int PC_NameHash( const char *name ) {
	int hash, i;
...
define_t *idParser::CopyDefine( define_t *define ) {
	...
	newdefine = (define_t *) mxAlloc(sizeof(define_t) + strlen(define->name) + 1);
...
void idParser::PushIndent( int type, int skip ) {
	...
	indent = (indent_t *) mxAlloc(sizeof(indent_t));
	indent->type = type;
...
void idParser::PushScript( idLexer *script ) {
	...
	for ( s = idParser::scriptstack; s; s = s->next ) {
		if ( !String::Icmp(s->GetFileName(), script->GetFileName()) ) {
...
int idParser::ReadSourceToken( idToken *token ) {
	...
	if ( !idParser::scriptstack ) {
		mxFatal( "idParser::ReadSourceToken: not loaded" );
		return false;
	}
	changedScript = 0;
...
void idParser::AddBuiltinDefines( void ) {
	...
	for (i = 0; builtin[i].string; i++) {
		define = (define_t *) mxAlloc(sizeof(define_t) + strlen(builtin[i].string) + 1);
...
#if 1
	UnimplementedX("The code below possibly has a bug");
	return false;
#else
	idToken *token;
	time_t t;
...
int idParser::Directive_define( void ) {
	...
	// allocate define
	define = (define_t *) mxAlloc(sizeof(define_t) + token.GetLength() + 1);
	MemZero( define, sizeof(define_t) + token.GetLength() + 1 );
	define->name = (char *) define + sizeof(define_t);
...
int idParser::DollarEvaluate( signed long int *intvalue, double *floatvalue, int integer) {
	...
	// The above code possibly has a bug.
	Unimplemented;
	return false;
...
=================
*/
const char *idParser::ParseBracedSectionExact( String &out, int tabs ) {
...
=================
*/
const char *idParser::ParseBracedSection( String &out, int tabs ) {
	idToken token;
	int i, depth;
	bool doTabs = false;
	if (tabs >= 0) {
...
=================
*/
const char *idParser::ParseRestOfLine( String &out ) {
	idToken token;
...
================
*/
int idParser::GetLastWhiteSpace( String &whiteSpace ) const {
	if ( scriptstack ) {
...
================
*/
void idParser::GetStringFromMarker( String& out, bool clean ) {
	char*	p;
	char	save;
...
void idParser::SetIncludePath( const char *path ) {
	...
	if (idParser::includepath[idParser::includepath.GetLength()-1] != '\\' &&
		idParser::includepath[idParser::includepath.GetLength()-1] != '/') {
...
int idParser::LoadFile( const char *filename ) {
	...
	if ( idParser::loaded ) {
		mxFatalf("idParser::loadFile: another source already loaded");
		return false;
	}
	script = new idLexer( filename, 0 );
	if ( !script->IsLoaded() ) {
		delete script;
		return false;
	}
	...
	if ( !idParser::definehash ) {
		...
		idParser::definehash = (define_t **) mxAlloc( DEFINEHASHSIZE * sizeof(define_t *) );
		MemZero( idParser::definehash, DEFINEHASHSIZE * sizeof(define_t *) );
		idParser::AddGlobalDefinesToSource();
	}
	return true;
}
...
int idParser::LoadMemory(const char *ptr, int length, const char *name ) {
	...
	if ( idParser::loaded ) {
		mxFatalf("idParser::loadMemory: another source already loaded");
		return false;
	}
...
================
*/
idParser::idParser( const char *filename, int flags ) {
	this->loaded = false;
	this->OSPath = true;
	this->punctuations = 0;
	this->flags = flags;

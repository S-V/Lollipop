...
FORCEINLINE idToken::idToken( void ) {
}

FORCEINLINE idToken::idToken( const idToken *token ) {
	*this = *token;
}
...
FORCEINLINE void idToken::operator=( const char *text) {
	*static_cast<String *>(this) = text;
}

FORCEINLINE void idToken::operator=( const String& text ) {
	*static_cast<String *>(this) = text;
}

FORCEINLINE double idToken::GetDoubleValue( void ) {
	if ( type != TT_NUMBER ) {
		return 0.0;
	}
...
FORCEINLINE float idToken::GetFloatValue( void ) {
	return (float) GetDoubleValue();
}

FORCEINLINE unsigned long	idToken::GetUnsignedLongValue( void ) {
	if ( type != TT_NUMBER ) {
		return 0;
	}
	if ( !(subtype & TT_VALUESVALID) ) {
...
FORCEINLINE int idToken::GetIntValue( void ) {
	return (int) GetUnsignedLongValue();
}

FORCEINLINE int idToken::WhiteSpaceBeforeToken( void ) const {
	return ( whiteSpaceEnd_p > whiteSpaceStart_p );
}

FORCEINLINE void idToken::AppendDirty( const char a ) {
	EnsureAlloced( mLength + 2, true );
	mData[mLength++] = a;
}

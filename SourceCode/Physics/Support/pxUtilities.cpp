...
idVec6 vec6_infinity( MX_INFINITY, MX_INFINITY, MX_INFINITY, MX_INFINITY, MX_INFINITY, MX_INFINITY );
...
=============
*/
void idWinding::BaseForPlane( const Vec3D &normal, const float dist ) {
...
=============
*/
int idWinding::Split( const Plane3D &plane, const float epsilon, idWinding **front, idWinding **back ) const {
	float *			dists;
	byte *			sides;
	int				counts[3];
	float			dot;
...
// RAVEN END
	...
	*back = b = new idWinding(maxpts);
	...
	if ( f->numPoints > maxpts || b->numPoints > maxpts ) {
		mxErrf( "idWinding::Split: points exceeded estimate." );
	}
...
=============
*/
idWinding *idWinding::Clip( const Plane3D &plane, const float epsilon, const bool keepOn ) {
	float *		dists;
	byte *		sides;
	idVec5 *	newPoints;
	int			newNumPoints;
	...
	counts[PLANESIDE_FRONT] = counts[PLANESIDE_BACK] = counts[PLANESIDE_ON] = 0;
	...
	if ( keepOn && !counts[PLANESIDE_FRONT] && !counts[PLANESIDE_BACK] ) {
		return this;
	}
	// if nothing at the front of the clipping plane
	if ( !counts[PLANESIDE_FRONT] ) {
	...
	newPoints = (idVec5 *) mxStackAlloc16( maxpts * sizeof( idVec5 ) );
	newNumPoints = 0;
...
=============
*/
bool idWinding::ClipInPlace( const Plane3D &plane, const float epsilon, const bool keepOn ) {
	float*		dists;
	byte *		sides;
	idVec5 *	newPoints;
	int			newNumPoints;
...
bool idWinding::Check( bool print ) const {
	int				i, j;
	float			d, edgedist;
	Vec3D			dir, edgenormal;
	float			area;
	Plane3D			plane;

	if ( numPoints < 3 ) {
		if ( print ) {
			mxPutf( "idWinding::Check: only %i points.", numPoints );
		}
		return false;
	}
	...
	if ( area < 1.0f ) {
		if ( print ) {
			mxPutf( "idWinding::Check: tiny area: %f", area );
		}
		return false;
	}
	...
	for ( i = 0; i < numPoints; i++ ) {
		const Vec3D &p1 = p[i].ToVec3();

		// check if the winding is huge
		for ( j = 0; j < 3; j++ ) {
			if ( p1[j] >= MAX_WORLD_COORD || p1[j] <= MIN_WORLD_COORD ) {
				if ( print ) {
					mxPutf( "idWinding::Check: point %d outside world %c-axis: %f", i, 'X'+j, p1[j] );
				}
				return false;
			}
		}
		...
		if ( d < -ON_EPSILON || d > ON_EPSILON ) {
			if ( print ) {
				mxPutf( "idWinding::Check: point %d off plane.", i );
			}
			return false;
		}
	
		// check if the edge isn't degenerate
		const Vec3D &p2 = p[j].ToVec3();
		dir = p2 - p1;
		
		if ( dir.GetLength() < ON_EPSILON) {
			if ( print ) {
				mxPutf( "idWinding::Check: edge %d is degenerate.", i );
			}
			return false;
		}
		...
		for ( j = 0; j < numPoints; j++ ) {
			...
			if ( d > edgedist ) {
				if ( print ) {
					mxPutf( "idWinding::Check: non-convex." );
				}
				return false;
			}
		}
...
=============
*/
float idWinding::GetRadius( const Vec3D &center ) const {
	int i;
	float radius, r;
	Vec3D dir;
...
=============
*/
Vec3D idWinding::GetCenter( void ) const {
	int i;
	Vec3D center;
...
=============
*/
Vec3D idWinding::GetNormal( void ) const {
	Vec3D v1, v2, center, normal;
...
=============
*/
void idWinding::GetPlane( Vec3D &normal, float &dist ) const {
	Vec3D v1, v2, center;
...
=============
*/
void idWinding::GetPlane( Plane3D &plane ) const {
	Vec3D v1, v2;
	Vec3D center;
...
=============
*/
void idWinding::GetBounds( AABB &bounds ) const {
	int i;
...
=============
*/
void idWinding::RemoveColinearPoints( const Vec3D &normal, const float epsilon ) {
	int i, j;
	Vec3D edgeNormal;
	float dist;
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( mxFabs( edgeNormal * p[(i+1)%numPoints].ToVec3() - dist ) > epsilon ) {
			continue;
		}
...
=============
*/
void idWinding::AddToConvexHull( const idWinding *winding, const Vec3D &normal, const float epsilon ) {
	int				i, j, k;
	Vec3D			dir;
	float			d;
	int				maxPts;
	...
	hullDirs = (Vec3D *) _alloca( maxPts * sizeof( Vec3D ) );
...
=============
*/
void idWinding::AddToConvexHull( const Vec3D &point, const Vec3D &normal, const float epsilon ) {
	...
	hullDirs = (Vec3D *) _alloca( numPoints * sizeof( Vec3D ) );
...
idWinding *idWinding::TryMerge( const idWinding &w, const Vec3D &planenormal, int keep ) const {
	Vec3D			*p1, *p2, *p3, *p4, *back;
	idWinding		*newf;
	...
	for ( i = 0; i < f1->numPoints; i++ ) {
		...
		for ( j = 0; j < f2->numPoints; j++ ) {
			...
			for (k = 0; k < 3; k++ ) {
				if ( mxFabs((*p1)[k] - (*p4)[k]) > 0.1f ) {
					break;
				}
				if ( mxFabs((*p2)[k] - (*p3)[k]) > 0.1f ) {
					break;
				}
			}
			if ( k == 3 ) {
...
void idWinding::RemovePoint( int point ) {
	if ( point < 0 || point >= numPoints ) {
		mxErrf( "idWinding::removePoint: point out of range" );
	}
...
=============
*/
void idWinding::InsertPoint( const Vec3D &point, int spot ) {
	int i;

	if ( spot > numPoints ) {
		mxErrf( "idWinding::insertPoint: spot > numPoints" );
	}

	if ( spot < 0 ) {
		mxErrf( "idWinding::insertPoint: spot < 0" );
	}
...
=============
*/
bool idWinding::InsertPointIfOnEdge( const Vec3D &point, const Plane3D &plane, const float epsilon ) {
	int i;
	float dist, dot;
	Vec3D normal;

	// point may not be too far from the winding plane
	if ( mxFabs( plane.Distance( point ) ) > epsilon ) {
		return false;
	}
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( mxFabs( normal * point - dist ) > epsilon ) {
			continue;
		}
...
=============
*/
float idWinding::PlaneDistance( const Plane3D &plane ) const {
	int		i;
	float	d, min, max;
...
=============
*/
int idWinding::PlaneSide( const Plane3D &plane, const float epsilon ) const {
	bool	front, back;
	int		i;
	float	d;
...
bool idWinding::PlanesConcave( const idWinding &w2, const Vec3D &normal1, const Vec3D &normal2, float dist1, float dist2 ) const {
	int i;
...
=============
*/
bool idWinding::PointInside( const Vec3D &normal, const Vec3D &point, const float epsilon ) const {
	int i;
	Vec3D dir, n, pointvec;
...
=============
*/
bool idWinding::LineIntersection( const Plane3D &windingPlane, const Vec3D &start, const Vec3D &end, bool backFaceCull ) const {
...
=============
*/
bool idWinding::RayIntersection( const Plane3D &windingPlane, const Vec3D &start, const Vec3D &dir, float &scale, bool backFaceCull ) const {
	int i;
...
=================
*/
float idWinding::TriangleArea( const Vec3D &a, const Vec3D &b, const Vec3D &c ) {
	Vec3D	v1, v2;
	Vec3D	cross;
...
bool idFixedWinding::ReAllocate( int n, bool keep ) {
	...
	if ( n > MAX_POINTS_ON_WINDING ) {
		mxPutf("WARNING: idFixedWinding -> MAX_POINTS_ON_WINDING overflowed\n");
		return false;
	}
	return true;
}
...
=============
*/
int idFixedWinding::Split( idFixedWinding *back, const Plane3D &plane, const float epsilon ) {
	int		counts[3];
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( !out.EnsureAlloced( out.numPoints+1, true ) ) {
			return PLANESIDE_FRONT;		// can't split -- fall back to original
		}
